...
import com.dishanhai.gt_shanhai.api.gui.configurators.MachineModeConfigurator;
...
public class PrimordialEngravingModule extends PrimordialOmegaEngineModuleBase {
    ...
    public PrimordialEngravingModule(IMachineBlockEntity holder, Object... args) {
        super(holder, args);
    ...
    protected NotifiableItemStackHandler[] getPersistedStorages() {
        return new NotifiableItemStackHandler[]{ machineStorage, threadBoostSlot, moduleSlot };
    }
    ...
    protected void addParallelDisplay(List<Component> textList) {
        ...
        textList.add(Component.literal("")
                .append(Component.literal("已安装: "))
                .append(itemName));

        textList.add(Component.literal("")
                .append(Component.literal("并行上限: "))
                .append(DShanhaiTextUtil.createNeonText(
                        currentParallel >= Long.MAX_VALUE ? "无限" : String.format("%,d", currentParallel))));

        long boost = getThreadBoost();
        if (boost > 0) {
            textList.add(Component.literal("")
                    .append(DShanhaiTextUtil.createElectricText("跨配方线程: "))
                    .append(DShanhaiTextUtil.createNeonText(String.format("%,d", boost))));
        }
    }
    ...
    public void addDisplayText(List<Component> textList) {
        if (isFormed()) {
            ...
            textList.add(Component.translatable("gt_shanhai.machine.primordial_engraving_module.mode").withStyle(ChatFormatting.GREEN));
        } else {
            textList.add(Component.translatable("gtceu.multiblock.invalid_structure")
                    .withStyle(ChatFormatting.RED));
        }
        textList.add(Component.translatable("gt_shanhai.machine.primordial_engraving_module.name")
