    ...
    public static class ConfigValues {
        ...
        void init(ForgeConfigSpec.Builder builder) {
            ...
            patternVirtualSupplyBatchParallel = builder
                    .comment("样板虚拟供料（首配路径）单个样板槽位「这一单」总共最多能给多少的预算（默认 65536）",
                             "注意：这不是单次提取的批量！单轮实际提取量另外受宿主机器并行上限封顶（每轮只拿这一轮吃得下的量），",
                             "此预算靠多轮反复补给慢慢用完，预算耗尽才算这一单彻底完成、不再自动补，不会一次性抽空网络仓库",
                             "调大能让「这一单」总量给得更多，调小更保守；不影响配方类型选择集本身的机器最大并行上限")
